...
function calcFare(vehicle, rawKm, tripType, numDays) {
    ...
    const minKm     = isRound ? vehicle.minRoundTrip * numDays : vehicle.minOneWay;
    const km        = isRound ? rawKm * 2 * numDays             : rawKm;
    ...
    return {
        ...
        actualKm:    Math.round(isRound ? rawKm * 2 : rawKm),
        rate,
        batta:       vehicle.batta,
        totalBatta,
...
function buildVehicleCard(vehicle, rawKm, tripType, pickup, drop, date, time, numDays) {
    ...
    <div class="fm-vehicle-card" style="
        ...
        <div style="padding:1.3rem 1.4rem; flex:1; display:flex; flex-direction:column; gap:0.55rem;">
            ...
            <div style="display:flex; justify-content:space-between; font-size:0.83rem; color:#475569; padding:1px 0;">
                <span>Rate / km</span><strong>₹${rate} / km</strong>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:0.83rem; color:#475569; padding:1px 0;">
        ...
        <div style="display:flex; flex-direction:column; gap:1rem;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                <div>
                    ...
                    <input id="bk-name" type="text" placeholder="Your full name" required
                ...
                <div>
                    ...
                    <input id="bk-phone" type="tel" placeholder="10-digit number" required
                        style="width:100%; padding:0.75rem 1rem; border:1.5px solid #e2e8f0; border-radius:10px; font-size:0.9rem; outline:none; font-family:inherit; box-sizing:border-box;"
                        onfocus="this.style.borderColor='#0077B6'" onblur="this.style.borderColor='#e2e8f0'">
                </div>
            </div>
...
async function handleQuickEstimate() {
    ...
    // Validate
    if (!pickup || !drop) {
        showError(errorEl, 'Please enter both Pickup and Drop locations.');
        return;
    }
    if (!date || !time) {
    ...
    try {
        const { km, text, duration } = await getGoogleDistance(pickup, drop);
                ...
                <div class="swiper-slide">
                    <div class="route-slide-card">
                        ...
                        <div class="route-card-body">
                            <a href="#contact" class="route-btn" data-from="${route.from}" data-to="${route.to}">Book Now</a>
        ...
        if (distEl)     distEl.textContent = text + (tripType === 'Round Trip' ? ` × ${numDays} day${numDays > 1 ? 's' : ''} = ${Math.round(km * 2 * numDays)} km billable` : '');
