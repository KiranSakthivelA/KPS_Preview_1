...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mainForm) {
        ...
        ['mb-pickup', 'mb-drop', 'mb-date', 'mb-end-date', 'mb-time'].forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('input', () => {
                    mbCachedRawKm = null;
        ...
        document.querySelectorAll('input[name="mb-trip-type"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                const isRound = e.target.value === 'Round Trip';
                const daysGroup = document.getElementById('mb-days-group');
                const endDateInput = document.getElementById('mb-end-date');
                if (daysGroup) daysGroup.style.display = isRound ? 'block' : 'none';
                if (endDateInput) {
                    endDateInput.disabled = !isRound;
                    if (!isRound) {
        ...
        mainForm.addEventListener('submit', async (e) => {
            ...
            let daysText = '';
            let dateText = `Date: ${document.getElementById('mb-date').value} ${document.getElementById('mb-time').value}`;
            
            if (tripType === 'Round Trip') {
                const days = parseInt(document.getElementById('mb-days').value) || 1;
                finalCar += ` (Round - ${days} Days)`;
                daysText = `\nDays: ${days}`;
                dateText = `Start: ${document.getElementById('mb-date').value}\nEnd: ${document.getElementById('mb-end-date').value}\nTime: ${document.getElementById('mb-time').value}`;
            } else {
                finalCar += ` (One Way)`;
            }
            ...
            const data = {
                ...
                endDate: tripType === 'Round Trip' ? document.getElementById('mb-end-date').value : '',
    ...
    const renderSliders = () => {
        ...
        document.querySelectorAll('.route-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const pickupInput = document.getElementById('mb-pickup');
                const dropInput = document.getElementById('mb-drop');
